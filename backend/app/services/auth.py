...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    try:
        ...
        user_identifier = payload.get('preferred_username') or payload.get('email') or payload.get('sub')
        logger.info(f"Successfully validated token for user: {user_identifier}")
        return payload
